...
ckan.SITE_ROOT=getRootFromData('siteRoot');ckan.LOCALE_ROOT=getRootFromData('localeRoot');ckan.sandbox().client.getLocaleData(locale).done(function(data){ckan.i18n.load(data);ckan.module.initialize();});if(jQuery.fn.popover!==undefined){jQuery('[data-target="popover"]').popover();}};ckan.url=function(path,includeLocale){if(typeof path==='boolean'){includeLocale=path;path=null;}
...
$.getJSON('http://data.bandung.go.id/c/countResources.php', function(data) {$('resourcecount').text(data)});
